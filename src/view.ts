 ...
 *
 * The read tools in this server hand back the skill registry's payload close to
 ...
 * honestly say which of the skill registry's fields matter and which are noise.
 *
 ...
 */
export const SK_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns the skill registry\'s payload untouched. ' +
  'No field projection: this server has no verified record of which the skill registry fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(SK_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, SK_VIEWS);
